...
import copy
from loguru import logger  # 日志文件加的,一般日志文件加在容易出错的地方
...
def from_file_get_data(file_name):
    ...
    try:
        ...
        logger.info("user_data:{}".format(user_data))  # 日志文件加的
        return user_data
    except Exception as  e:
        # print(e)
        logger.error("出现异常，异常内容:{}".format(e))  # 日志文件加的
    finally:
        if not f:
            f.close()  # 这里的f可能是None,就不能调用close()方法


# 方法：能向文件中写入内容，用户添加的信息是在原来的基础上添加的。
def save_data(file_name, file_content):
    f = None
    try:
        f = open(file_name, 'w')
        f.write(str(file_content))  # file_content是一个列表，而write保存的是字符串
    except Exception as e:
        print(e)
    finally:
        if not f:
...
值 if 表达式1  else 表达式2
"""
# 如果user_list有值直接返回if前面的user_list的值否则的话调用from_file_get_data('user_data.txt')
user_list = user_list if user_list else from_file_get_data(r'D:\project\pyhon_01\homework\user_data.txt')
...
class User():
    ...
    # 添加用户的方法
    def add_user(self, username, password='123456', **kwargs):  # **代表以字典的形式传
        user_dict = {}
    ...
    # 用户查询功能
    def get_user(self, username):  # 放在类当中前面一定要加self占位符
        # 判断用户是否登录，若登陆成功可以进行查询：若登陆失败，返回权限不足
        ...
        for x in self.user_lst:
            account = x.get('account')  # 等号是精确查找，in是模糊查找
            logger.info("account:{}".format(account))  # 日志文件加的
            if username in account:  # 模糊查询
                if x.get('password'):
                    x.pop('password')  # 去除 password对应的值
                lst.append(x)
...
if __name__ == '__main__':
    ...
    flag = True
    while flag:
        # 提供用户的各种选择，比如输入1代表登录，输入2代表查询操作，输入q:退出操作 # \n代表换行
        vls = input("操作请输入对应编号:"
                    "\n 1:)  进行登录 :"
                    "\n 2:)  进行查询用户，请输入用户名:"
                    "\n 3:)  添加用户，请输入用户信息:"
        ...
        if vls == '2':
            name = input("请输入查询用户名：")
            user = User()
            get_result = user.get_user(name)  # 以前是个函数现在是个类的方法
            print(get_result)
            print("=" * 30)
            continue
        # 添加用户的情况
        if vls == '3':
            ...
            if 12 == get_result.get('code'):  # 12代表没有查询到用户
                password = input("请输入用户密码:")
                role = input("请输入用户角色:")
                dept = input("请输入用户部门:")
                get_result = user.add_user(name, password, role=role, dept=dept)  # 以关键字形式传值=，用字典的话是：
            if 10 == get_result.get('code'):
                get_result = ({'code': 13, 'message': "用户已存在,无法添加"})
            print(get_result)  # 打印查询结果
            print("=" * 30)
